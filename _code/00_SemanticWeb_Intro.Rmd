...
car_triples$object <- sapply(car_triples$object,
                             function(x) {
                               if (grepl('^[[:digit:]]+$', x)) {
                                 ...
                                 return(paste0('"', x, '"', '^^', '<http://www.w3.org/2001/XMLSchema#double>'))
...
# - add one triple
triplestore %>% 
  rdf_add(subject = paste0(prefix_mtcars, car_triples$subject[1]), 
          predicate = paste0(prefix_mtcars, car_triples$predicate[1]), 
...
# - add another one
triplestore %>% 
  rdf_add(subject = paste0(prefix_mtcars, car_triples$subject[2]), 
          predicate = paste0(prefix_mtcars, car_triples$predicate[2]), 
...
**Note.** We can now see an automatic type conversion from R to `xsd:`, however it is not perfect.
...
leaflet() %>%
        ...
        addCircleMarkers(lng = citiesMap$lng,
                         lat = citiesMap$lat)
leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
